#!/usr/bin/env python3
"""MaxOS AI Developer v17.0 — Architecture redessinée, taux de succès x10"""
...
# ── KNOWLEDGE BASE — La mémoire persistante du bot ────────────────────────────
# Contient les signatures canoniques de toutes les fonctions du projet.
# L'IA DOIT utiliser ces signatures exactes. Jamais en inventer.
...
# Erreurs connues et leurs solutions hardcodées
KNOWN_FIXES = {
    "undefined reference to `isr": (
        "isr.asm DOIT avoir: 'global isr0' jusqu'à 'global isr47' ET être dans Makefile OBJS. "
        "Vérifier que $(BUILD)/isr.o est dans la liste des objets de kernel.bin."
    ),
    "undefined reference to `main'": (
        "kernel_entry.asm appelle 'kmain' pas 'main'. "
        "Changer 'extern main' en 'extern kmain' et 'call main' en 'call kmain'."
    ),
    "undefined reference to `kernel_main'": (
        "La fonction d'entrée s'appelle 'kmain' dans kernel.c. "
        "Remplacer toute référence à 'kernel_main' par 'kmain'."
    ),
    "conflicting types for 'fault_handler'": (
        f"fault_handler doit avoir la signature EXACTE: {CANONICAL_SIGNATURES['fault_handler']}. "
        "Vérifier que fault_handler.h déclare exactement cette signature."
    ),
    "conflicting types for 'screen_write'": (
        f"screen_write doit avoir la signature EXACTE: {CANONICAL_SIGNATURES['screen_write']}. "
        "Vérifier toutes les déclarations dans screen.h et screen.c."
    ),
    "conflicting types for 'vga_putchar'": (
        f"vga_putchar doit avoir la signature EXACTE: {CANONICAL_SIGNATURES['screen_putchar']} (adaptée pour vga). "
        "Unifier les signatures dans vga.h et vga.c."
    ),
    "screen.h: No such file or directory": (
        "screen.h est dans drivers/. Utiliser #include \"drivers/screen.h\" ou corriger -I dans CFLAGS. "
        "Le Makefile doit avoir CFLAGS avec -I. pour trouver les headers."
    ),
    "No rule to make target": (
        "Un fichier .o est référencé dans Makefile mais le .c/.asm source n'existe pas. "
        "Retirer les objets orphelins du Makefile ou créer les fichiers sources manquants."
    ),
    "bad register name `%eip'": (
        "En mode 32-bit NASM, on ne peut pas pousser EIP directement. "
        "Retirer 'push eip' ou utiliser 'call $+5 / pop eax' pour obtenir EIP."
    ),
    "too many arguments to function 'fault_handler'": (
        f"fault_handler signature = {CANONICAL_SIGNATURES['fault_handler']}. "
        "isr.c doit appeler fault_handler(num, err) avec exactement 2 arguments."
    ),
    "too few arguments to function 'screen_write'": (
        f"screen_write signature = {CANONICAL_SIGNATURES['screen_write']}. "
        "Passer 2 arguments: la string ET la couleur."
    ),
    "unknown type name 'task_t'": (
        "Inclure kernel/task.h avant d'utiliser task_t. "
        "Vérifier l'ordre des includes dans timer.c."
    ),
    "undeclared.*task_list": (
        "task_list doit être déclaré extern dans task.h et défini dans task.c. "
        "Inclure task.h dans timer.c."
    ),
    "undefined reference to `idt_set_gate'": (
        "idt.c doit être compilé et listé dans Makefile OBJS. "
        "Vérifier que $(BUILD)/idt.o est dans la règle kernel.bin."
    ),
    "undefined reference to `screen_write'": (
        "drivers/screen.c doit être dans Makefile SRCS_C. "
        "Vérifier VPATH inclut 'drivers' et screen.o est dans OBJS."
    ),
}
...
def penalize(p, secs=None, dead=False):
    if dead:
        p["dead"] = True; _CYCLE_STATS["providers_dead"] += 1
        log(f"Provider {p['id']} → MORT", "ERROR"); return
    p["errors"]      += 1
    p["consec_429"]  += 1
...
def pick():
    ...
    if al:
        chosen = al[0]
        if DEBUG: log(f"  pick → {chosen['id']} sr={chosen['success_rate']:.2f}")
        return chosen
    nd = non_dead()
    if not nd:
...
def _call_gemini(p, prompt, max_tok, timeout):
    payload = json.dumps({
        ...
        "generationConfig": {
            "maxOutputTokens": max_tok, "temperature": 0.05, "candidateCount": 1
        },
    }).encode("utf-8")
...
def ai_call(prompt, max_tokens=32768, timeout=160, tag="?"):
    ...
    for attempt in range(1, max_att+1):
        ...
        try:
            ...
            elif e.code == 404: penalize(p, dead=True)
            elif e.code == 400:
...
def _disc_raw(embeds):
    ...
    payload = json.dumps({
        "username": f"MaxOS AI v{VERSION}", "embeds": embeds[:10]
    }).encode()
    req = urllib.request.Request(
...
def git_push(task_name, files, desc, model):
    ...
    if not ok:
        if "nothing to commit" in (out+err):
            log("Git: rien à committer"); return True, None, None
    ...
    if not ok2:
        ...
        if not ok2:
            log(f"Push KO: {e2[:250]}", "ERROR"); return False, None, None
...
def _detect_silent_fail(log_text, returncode):
    ...
    for line in lines:
        s = line.strip()
        if re.search(r"Error \d+", s) or "FAILED" in s: return s[:140]
        if "make[" in s and "Error" in s:               return s[:140]
        if "*** " in s:                                  return s[:140]
...
def _create_osimg(boot_bin, kernel_bin=None):
    ...
    try:
        ...
        log(f"Erreur création os.img: {e}", "ERROR"); return False
...
# ── SNAPSHOT — État stable du projet ─────────────────────────────────────────
class ProjectSnapshot:
    """
    Capture l'état stable du projet:
    - Signatures de toutes les fonctions
    - Liste des symboles exportés par chaque fichier
    - Contenu du Makefile
    - Fichiers existants
    Permet de détecter les incohérences AVANT de lancer l'IA.
    """

    def __init__(self, sources):
        self.sources    = sources
        self.signatures = {}   # fname → {func_name: signature}
        self.exports    = {}   # fname → [symbol, ...]
    ...
    def get_isr_globals(self):
        """Vérifie que isr.asm exporte bien isr0..isr47."""
    ...
    def check_consistency(self):
        """
        Retourne une liste de problèmes détectés dans le projet actuel.
        Ces problèmes sont injectés dans le prompt pour que l'IA les évite.
        """
        issues = []

        # 1. Vérifier que kmain existe
        kmain_found = False
        for fname, sigs in self.signatures.items():
            if "kmain" in sigs:
                kmain_found = True; break
        if not kmain_found:
            issues.append("⚠️ 'kmain' introuvable — kernel_entry.asm doit appeler 'kmain'")

        # 2. Vérifier isr.asm globals
        isr_globals = self.get_isr_globals()
        missing_isr = []
        for i in range(48):
            if f"isr{i}" not in isr_globals:
                missing_isr.append(f"isr{i}")
        if missing_isr:
            issues.append(f"⚠️ isr.asm manque: {missing_isr[:5]}... ({len(missing_isr)} total)")

        # 3. Vérifier cohérence des signatures avec CANONICAL_SIGNATURES
        all_sigs = self.get_all_func_signatures()
        for func, canonical in CANONICAL_SIGNATURES.items():
            if func in all_sigs:
                actual = all_sigs[func]
                # Extraire le nom+args pour comparer (simpliste mais utile)
                if func not in actual:
                    issues.append(f"⚠️ Signature suspecte pour '{func}': {actual[:80]}")

        # 4. Vérifier Makefile contient os.img
    ...
    def get_context_for_task(self, task, max_chars=26000):
        """
        Retourne le contexte minimal mais complet pour une tâche donnée.
        Inclut TOUJOURS: Makefile, io.h, les fichiers concernés, et les headers des dépendances.
        """
        needed = set()
        needed.update(task.get("fichiers_a_modifier",[]))
        needed.update(task.get("fichiers_a_creer",[]))

        # Ajouter les headers correspondants
        for f in list(needed):
            if f.endswith(".c"):   needed.add(f.replace(".c",".h"))
            elif f.endswith(".h"): needed.add(f.replace(".h",".c"))

        # Toujours inclure les fichiers critiques
        ...
        ctx = ""; used = 0
        # Priorité aux fichiers directement concernés
...
SRCS_C = kernel/kernel.c kernel/idt.c kernel/timer.c kernel/memory.c \\
         kernel/fault_handler.c kernel/exceptions.c \\
...
def diagnose_errors(errs, build_log, snap):
    """
    Analyse les erreurs et retourne:
    - diagnostics: solutions connues
    - corrupted_files: fichiers probablement à recréer
    - makefile_broken: bool
    - needs_canonical_reset: bool
    """
    err_str        = "\n".join(errs)
    diagnostics    = []
    corrupted      = set()
    ...
    # Parcourir KNOWN_FIXES
    for pattern, solution in KNOWN_FIXES.items():
        if re.search(pattern, err_str, re.IGNORECASE):
            diagnostics.append(f"⚡ {solution}")

    # Fichiers corrompus probable
    # Extraire les fichiers mentionnés dans les erreurs
    file_re = re.compile(r"(\w[\w/\.]+\.(?:c|h|asm|s))")
    mentioned = set(file_re.findall(err_str))
    for f in mentioned:
        # Si l'erreur pointe vers un fichier qui ne devrait pas exister
        if any(bad in f for bad in ["unsigned_char","kernel_main","v_put","v_str"]):
            corrupted.add(f)

    # Détecter les problèmes de nommage inventés par l'IA
    invented_syms = ["kernel_main","v_put","v_str","unsigned_char","screen_puthex","screen_putstr"]
    for sym in invented_syms:
        if sym in err_str:
            canon = CANONICAL_SIGNATURES.get(sym.replace("kernel_main","kmain"), "")
            diagnostics.append(
                f"⚡ SYMBOLE INVENTÉ '{sym}' — utiliser le nom canonique"
    ...
    # Reset si trop d'erreurs non diagnosticables
    if len(errs) > 20 and not diagnostics:
        needs_reset = True
        diagnostics.append("⚡ Trop d'erreurs non diagnosticables — reset Makefile recommandé")
...
def _build_signatures_block(snap):
    """
    Construit le bloc de signatures canoniques à injecter dans le prompt.
    Montre l'état réel du projet + les signatures attendues.
    """
...
# ── ANALYSE ───────────────────────────────────────────────────────────────────
def phase_analyse(context, stats, snap):
    ...
    prompt = (
        ...
        '"description":"specs précises et complètes","impact_attendu":"r","complexite":"HAUTE"}]}'
    )
...
def impl_prompt(task, ctx, snap, consistency_issues):
    ...
    return (
        ...
        "8. NE PAS inventer de noms de fonctions — utiliser les signatures canoniques ci-dessus\n"
        "9. 'unsigned char' JAMAIS 'unsigned_char' (underscore interdit)\n"
        "10. Pas de 'push eip' ou 'mov %eip' en mode 32-bit\n"
        "11. screen_write prend 2 args: (const char*, unsigned char)\n"
        "12. fault_handler prend 2 args: (unsigned int num, unsigned int err)\n\n"
        "FORMAT DE RÉPONSE:\n"
...
def auto_fix(build_log, errs, gen_files, bak, model, snap, max_att=4):
    ...
    for att in range(1, max_att+1):
        ...
        # Si Makefile cassé → reset immédiat
        if diag["makefile_broken"]:
            log("Fix: Makefile cassé → reset canonique", "WARN")
            _ensure_build_system()

        # Récupérer les fichiers actuels concernés
        curr_files = {}
        for p in gen_files:
        ...
        # Ajouter aussi les fichiers critiques qui pourraient être la cause
        ...
        prompt = (
            ...
            "RÈGLES FIX SPÉCIALES:\n"
            "- Si 'undefined reference to isr0...' → réécrire isr.asm complet avec global isr0..isr47\n"
            "- Si 'undefined reference to main' → kernel_entry.asm doit appeler 'kmain'\n"
            "- Si 'conflicting types' → aligner signature avec le bloc SIGNATURES CANONIQUES\n"
            "- Si 'No such file or directory' pour un .h → corriger le chemin #include\n"
            "- Si 'No rule to make target' → corriger Makefile SRCS_C et VPATH\n"
            "- Si 'bad register name %eip' → supprimer cette ligne, pas utilisable en 32-bit\n\n"
            "FORMAT:\n=== FILE: fichier ===\n[code]\n=== END FILE ==="
        )

        resp = ai_call(prompt, max_tokens=32768, timeout=130, tag=f"fix/{att}")
        if not resp:
            time.sleep(min(8*(2**(att-1)), 45)); continue

        new_files, _ = parse_ai_files(resp)
        if not new_files:
            log(f"Fix {att}: parse vide", "WARN")
            time.sleep(min(5*(2**(att-1)), 30)); continue
        ...
        # Anti-boucle: si les mêmes erreurs reviennent, changer d'approche
        if att >= 2:
            prev_key = sorted(cur_errs)
            if hasattr(auto_fix, '_last_errs') and sorted(auto_fix._last_errs) == prev_key:
                log("Fix: mêmes erreurs en boucle — reset Makefile + attente", "WARN")
                _ensure_build_system(); time.sleep(3)
        auto_fix._last_errs = cur_errs[:]
...
def pre_flight_check():
    ...
    # Tenter de réparer immédiatement avec IA
...
def implement(task, sources, snap, i, total):
    ...
    # Build échoué → auto-fix avec snapshot
...
def handle_prs():
    ...
    for pr in prs[:5]:
        ...
        if decision == "APPROVE" and merge_safe:
            ...
            gh_req_changes(n, body); gh_add_labels(n, ["ai-rejected","ai-reviewed"])
        else:
            gh_post_review(n, body, "COMMENT"); gh_add_labels(n, ["ai-reviewed"])
...
def create_release(tasks_done, tasks_failed, analyse, stats):
    ...
    if release_data:
        ...
        if img_ok and release_id:
            ...
            if asset_url: log(f"os.img uploadé: {asset_url}", "OK")
...
def main():
    ...
    print(f"  Architecture redessinée | Snapshot | Knowledge Base")
    print("="*64)
    ...
    disc_now(f"🤖 MaxOS AI v{VERSION} — Démarrage",
             f"`{len(PROVIDERS)}` providers | Knowledge Base activée", 0x5865F2,
             [{"name":"🔑 Providers","value":prov_summary()[:800],"inline":False},
              ...
              {"name":"🧠 Signatures","value":f"{len(CANONICAL_SIGNATURES)} canoniques","inline":True}])
    ...
    if consistency:
        ...
        for issue in consistency: log(f"  {issue}", "WARN")
    ...
    analyse   = phase_analyse(build_ctx(sources), stats, snap)
    score     = analyse.get("score_actuel", 35)
    ...
    for i, task in enumerate(plan, 1):
        if not watchdog():
            log(f"Watchdog: arrêt avant tâche {i}/{total}", "WARN"); break
        if remaining_time() < 200:
            log("Moins de 200s restantes — arrêt", "WARN"); break

        disc_log(f"💓 [{i}/{total}] {task.get('nom','?')[:45]}",
                 f"Uptime: {uptime()} | Reste: {int(remaining_time())}s\n{prov_summary()[:250]}",
                 0x7289DA)

        # Snapshot frais avant chaque tâche

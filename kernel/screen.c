...
void screen_clear(void) {
    for (unsigned int i = 0; i < VGA_WIDTH * VGA_HEIGHT; i++) {
        video_memory[i] = (unsigned short)0x20 | (color << 8);
    }
    row = 0;
    col = 0;
}
...
void screen_putchar(char c, unsigned char color) {
    if (c == '\n') {
        ...
        video_memory[row * VGA_WIDTH + col] = (unsigned short)c | (color << 8);
        col++;
    }
...
void screen_scroll(void) {
    ...
    for (unsigned int j = 0; j < VGA_WIDTH; j++) {
        video_memory[(VGA_HEIGHT - 1) * VGA_WIDTH + j] = (unsigned short)0x20 | (color << 8);
    }

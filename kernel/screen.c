...
void screen_putchar(char c, unsigned char color)
{
    ...
    unsigned short *pos = (unsigned short *)video_memory + (row * MAX_COLS + col);
...
void screen_scroll(void)
{
    for (int i = 0; i < MAX_ROWS - 1; i++) {
        for (int j = 0; j < MAX_COLS; j++) {
            unsigned short *src = (unsigned short *)video_memory + ((i + 1) * MAX_COLS + j);
            unsigned short *dest = (unsigned short *)video_memory + (i * MAX_COLS + j);
            *dest = *src;
        }
    }

    for (int j = 0; j < MAX_COLS; j++) {
        unsigned short *pos = (unsigned short *)video_memory + ((MAX_ROWS - 1) * MAX_COLS + j);
        *pos = (color << 8) | ' ';
    }
